...
def val(net:torch.nn.Module, valIter, device, loss, epoch, epochNum, resultPath):
    """验证"""
    ...
    for i, (X, y) in enumerate(valIter):
        ...
        print(f"\r\033[K\033[31mepoch\033[0m {epoch:>3}/{epochNum}    \033[31mbatch:\033[0m{i+1}/{batchNum}    \033[31mprogress:\033[0m{prg}    \033[31msample_num:\033[0m{X.shape[0]}    \033[31mval_loss:\033[0m{val_loss:.5f}", end="\r")
        if prg == '100%':
            time.sleep(0.1)
        ...
        with open(valIterPath, 'a+') as tIter_fp:
            tIter_fp.write(
                f"{epoch:>6}\t{i+1:>6}\t{sampleNum:>6}\t{correctNum:>6}\t{batchAcc:>6.4}\t{batchLoss:>6.2f}\t{val_loss:>6.3f}\n")
