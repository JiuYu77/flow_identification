...
function drawCurve(ctx, data, minValue, valueRange, chartWidth, chartHeight, padding, config) {
    ...
    if (config.smooth) {
        // 绘制平滑曲线
        drawSmoothCurve(ctx, data, minValue, valueRange, chartWidth, chartHeight, padding, pointSpacing);
    } else {
        // 绘制折线
        drawLinearCurve(ctx, data, minValue, valueRange, chartHeight, padding, pointSpacing);
    }

    // 绘制数据点
    if (config.showPoints) {
        drawDataPoints(ctx, data, minValue, valueRange, chartHeight, padding, pointSpacing, config);
    }
}
...
function drawSmoothCurve(ctx, data, minValue, valueRange, chartWidth, chartHeight, padding, pointSpacing) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (i === 0) {
            ...
            const prevY = padding.top + chartHeight - ((data[i - 1] - minValue) / valueRange) * chartHeight;
