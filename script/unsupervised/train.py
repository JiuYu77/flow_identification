...
def train(dataset,
    ...
    net = YOLO1Dv8(modelYaml, model, fuse_=fuse_, split_=split_, initweightName=initweightName, scale=scale, device=device)  # 实例化模型
    ...
    resultPath = os.path.join(ROOT, 'result', 'train_unsupervised', thisDir)
    weightsPath = os.path.join(resultPath, "weights")  # 模型参数文件 文件夹
    ph.checkAndInitPath([resultPath, weightsPath])
    info_fp_path = os.path.join(resultPath, 'info.yaml')  # 通过yaml文件记录模型数据
    bestWeightPath = os.path.join(weightsPath, "best_params.pt")  # 模型参数文件
    lastWeightPath = os.path.join(weightsPath, "last_params.pt")  # 模型参数文件
    ...
    for epoch in range(epochNum):
        ...
        for i, (X, y) in enumerate(trainIter):
            ...
            if type(optimizer) == optim.SGD:  # 如果优化算法是随机梯度下降
                with torch.no_grad():
                    accumulatorTrain.add(batchLoss, sampleNum)
            ...
            with open(trainIterPath, 'a+') as tIter_fp:
                tIter_fp.write(
                    f"{epoch:>6}\t{i+1:>6}\t{sampleNum:>6}\t{batchLoss:>6.2f}\t{train_loss:>6.3f}\n")

        # 一个epoch的 训练损失 和 准确率
        trainLoss = accumulatorTrain[0] / accumulatorTrain[1]  # 每个样本 平均损失
        ...
        #     bestAccSymbol = '  *'
        if epoch == int(epochNum * 0.5) or epoch == int(epochNum * 0.65) or \
            epoch == int(epochNum * 0.75) or epoch == int(epochNum * 0.85) or epoch == int(epochNum * 0.95):
            p = os.path.join(weightsPath, f"{epoch}_params.pt")
            torch.save(net.state_dict(), p)  # 保存网络参数
            saveSymbol = '  *'
        if epoch + 1 == epochNum:
        ...
        with open(epochPath, 'a+') as f:
            f.write(f"epoch {epoch_}    train_loss: {trainLoss:.5f}    val_acc: {saveSymbol}\n")
    ...
    print(f"| loss {trainLoss:.4f}")
    # print(f"| loss {trainLoss:.4f}, train_acc {trainAcc:.4f}, val_acc {valAcc:.4f}")
    print(f"| {accumulatorTrain[1] * epochNum / timer.sum():.1f} samlpes/sec, on \033[33m{str(device)}\033[0m")  # 每秒处理的样本数, 使用的设备
