...
def combine(all:list, data, label):
...
tranRadio = 0.7
valRadio = 0.15
testRadio = 1 - tranRadio - valRadio

trainAll = []
tmp = tranRadio * counts
tranCounts = [int(i) for i in tmp]
samples = data[:tranCounts[0]]
labels = label[:tranCounts[0]]
trainAll = combine(trainAll, samples, labels)
print(">>>>>>>", samples.shape, samples[0].shape)

begin = counts[0]
samples = data[begin:begin+tranCounts[1]]
labels = label[begin:begin+tranCounts[1]]
trainAll = combine(trainAll, samples, labels)

valAll = []
tmp = valRadio * counts
valCounts = [int(i) for i in tmp]
samples = data[tranCounts[0]:valCounts[0]]
labels = label[tranCounts[0]:valCounts[0]]
valAll = combine(valAll, samples, labels)

begin = counts[0]+tranCounts[1]
samples = data[begin:begin+valCounts[1]]
labels = label[begin:begin+valCounts[1]]
valAll = combine(valAll, samples, labels)

print(samples)

testAll = []
testCounts = tmp = [int(v-tranCounts[i]-valCounts[i]) for i,v in enumerate(counts)]
samples = data[tranCounts[0]+valCounts[0]:int(counts[0])]
labels = label[tranCounts[0]+valCounts[0]:int(counts[0])]
testAll = combine(testAll, samples, labels)

begin = counts[0]+tranCounts[1]+valCounts[1]
samples = data[begin:begin+int(counts[1])]
labels = label[begin:begin+int(counts[1])]
testAll = combine(testAll, samples, labels)
...
def train(dataset,
    ...
    trainIter.dataset.allSample = trainAll
    valIter.dataset.allSample = valAll
    ...
    net = YOLO1Dv8(modelYaml, model, fuse_=fuse_, split_=split_, scale=scale, initweightName=initweightName, device=device)  # 实例化模型
